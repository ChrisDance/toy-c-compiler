...
import { Parser, NodeType, Program } from '../src/parser';
...
describe('Parser', () => {
  ...
  test('should parse a simple function', () => {
    ...
    expect(returnStmt.argument.type).toBe(NodeType.NumberLiteral);
    expect((returnStmt.argument as any).value).toBe(42);
  });
  ...
  test('should parse if statements', () => {
    ...
    expect((ifStmt as any).thenBranch.statements[0].type).toBe(NodeType.ReturnStatement);
    ...
    expect((ifStmt as any).elseBranch.statements[0].type).toBe(NodeType.ReturnStatement);
  });
